...
const getDefaultTitle = (): string => {
  switch (shareType.value) {
    case 'chapter':
      const chapter = series.value.flatMap(s => s.chapters).find(c => c.id === selectedChapterId.value);
...
const selectAllChapters = () => {
  selectedChapterIds.value = series.value.flatMap(s => s.chapters)
...
const selectAllSeries = () => {
  selectedSeriesIds.value = series.value.filter(s => s.chapters.some(c => c.originalParagraphs.some(p => p.trim()))).map(s => s.id);
};
...
const toggleSeriesSelection = (seriesId: string) => {
  const seriesChapters = (series.value.find(s => s.id === seriesId)?.chapters || [])
    .filter(c => c.originalParagraphs.some(p => p.trim()))
  );
